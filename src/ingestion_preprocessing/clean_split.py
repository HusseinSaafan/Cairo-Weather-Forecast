...
- Test set is the last 365 days in the source data.
...
def main() -> None:
	...
	train_df = clean_df.loc[date_index < test_start_date].copy()

	if test_df.empty:
		raise ValueError("Test split is empty. Source data does not include the last 365-day window.")
	if train_df.empty:
		raise ValueError("Train split is empty. No rows exist before the test window.")
